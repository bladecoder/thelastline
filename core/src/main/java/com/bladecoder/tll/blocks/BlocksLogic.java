...
public class BlocksLogic {
  ...
  private void lockDown() {
    ...
    if (gameState.playfield.isSomeRowFull()) {
      ...
      if (gameState.tetramino.getPos().y > 2) {
        // Pieces that lock in the bottom two rows are followed by 10 frames of entry delay, and
        // each group of 4
  ...
  public boolean moveLeft() {
    if ((gameState.state != GameState.State.FALLING && gameState.state != GameState.State.ARE)
      ...
      for (int x = 0; x < gameState.tetramino.getWidth(); x++) {
        if (!gameState.tetramino.isFree(x, y)) {
          if (!gameState.playfield.isFree(
              (int) gameState.tetramino.getPos().x + x - 1,
              (int) gameState.tetramino.getPos().y + y)) return false;
        }
      }
    }
