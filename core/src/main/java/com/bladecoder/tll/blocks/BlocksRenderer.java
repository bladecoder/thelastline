...
public class BlocksRenderer {
  ...
  public void resize(int width, int height) {
    ...
    if (playfieldBorderWidth < 1) playfieldBorderWidth = 1;

    scoresBorderWidth = (int) (theme.scoresBorderWidth * scale);

    if (scoresBorderWidth < 1) scoresBorderWidth = 1;

    float playfieldAspect =
        (float) gameState.playfield.getWidth() / (float) gameState.playfield.getHeight();
    ...
    if (vertical) {
      ...
      this.org.set(
          (float) ((width - playfieldWidth) / 2.0),
          DPIUtils.getMarginSize() + playfieldBorderWidth);
  ...
  public void render(SpriteBatch batch) {
    ...
    RectangleRenderer.draw(
        batch,
        org.x - playfieldBorderWidth,
        org.y - playfieldBorderWidth,
        playfieldWidth + playfieldBorderWidth * 2,
        playfieldHeight + playfieldBorderWidth * 2,
        theme.playfieldColor,
        playfieldBorderWidth,
        theme.playfieldBorderColor);

    // render grid
    if (theme.gridColor != null) renderGrid(batch);
    ...
    if (gameState.state == GameState.State.LEVEL_UP) {
      RectangleRenderer.draw(
          ...
          org.x,
          org.y,
          tileSize * gameState.playfield.getWidth(),
          tileSize * gameState.playfield.getHeight(),
          theme.clearLineColor);
    }
    ...
    for (int y = 0; y < gameState.playfield.getHeight(); y++) {
      for (int x = 0; x < gameState.playfield.getWidth(); x++) {
        if (gameState.playfield.get(x, y) != 0) {
          renderTile(
              batch, org.x + x * tileSize, org.y + y * tileSize, gameState.playfield.get(x, y));
        }
      }
    }
    ...
    // render scores
    if (vertical) renderScoresV(batch);
    else renderScoresH(batch);
  }
    ...
    for (int y = 0; y < gameState.tetramino.getCurrent().length; y++) {
      for (int x = 0; x < gameState.tetramino.getCurrent()[y].length; x++) {
        if (!gameState.tetramino.isFree(x, y)
            && gameState.tetramino.getPos().y + y < gameState.playfield.getHeight()) {
          renderTile(
              batch, posX + x * tileSize, posY + y * tileSize, gameState.tetramino.get(x, y));
        }
      }
    }
  }
  ...
  private void renderScoresH(SpriteBatch batch) {
    ...
    float posy =
        org.y + gameState.playfield.getHeight() * tileSize - scoresHeight + playfieldBorderWidth;
    ...
    scoresWidth2 =
        renderScoreText(
            batch, posx, posy, scoresWidth2, scoresHeight, titleStr, valueStr, highScoreColor);
  ...
  private void renderScoresV(SpriteBatch batch) {
    ...
    float posy =
        org.y
            + gameState.playfield.getHeight() * tileSize
            + playfieldBorderWidth
            + scoresHeight
            - scoresBorderWidth * 2;
    ...
    scoresWidth1 =
        renderScoreText(
            batch, posx, posy, scoresWidth1, scoresHeight, titleStr, valueStr, highScoreColor);
    ...
    if (scoresBorderWidth < 1 || theme.scoresBorderColor == null) posx += DPIUtils.getMarginSize();
  ...
  private void renderNextTetramino(
      SpriteBatch batch, float posx, float posy, float width, float height) {
  ...
  private float renderScoreText(
    ...
    if (value == null) {
      ...
      smallFont.draw(
          batch, textLayoutSmall, posx + width / 2, posy + (height + textLayoutSmall.height) / 2);
      return finalWidth;
    }
    ...
    smallFont.draw(
        ...
        posx + width / 2,
        posy
            + (height + textLayoutSmall.height + textLayoutBig.height + DPIUtils.getMarginSize())
                / 2);
    bigFont.draw(
        batch,
        textLayoutBig,
        posx + width / 2,
        posy
            + (height + textLayoutBig.height - textLayoutSmall.height - DPIUtils.getMarginSize())
                / 2);
  ...
  private void renderTile(SpriteBatch batch, float x, float y, int type) {
    if (tile != null) {
      ...
      RectangleRenderer.draw(
          ...
          tileSize,
          tileSize,
          theme.tileColors[(type - 1) % theme.tileColors.length],
          theme.tileBorderWidth * scale,
          theme.tileBorderColor);
  }
